...
class SimpleStorage private constructor(private val wrapper: ComponentWrapper) {
    ...
    fun requestStorageAccess(
        ...
        if (initialRootPath == StorageType.EXTERNAL && expectedStorageType.isExpected(
                initialRootPath
            ) && Build.VERSION.SDK_INT < Build.VERSION_CODES.Q && !isSdCardPresent
        ) {
        ...
        val intent =
            if (initialRootPath == StorageType.SD_CARD && Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R || DocumentFileCompat.isRootUri(uri)) {
            ...
            if (storageId == PRIMARY) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N && Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
                    val sm = context.getSystemService(Context.STORAGE_SERVICE) as StorageManager
                    @Suppress("DEPRECATION")
                    sm.storageVolumes.firstOrNull { it.isRemovable }?.createAccessIntent(null)
                        ?.let {
    ...
    companion object {
            ...
            get() = Intent(Intent.ACTION_OPEN_DOCUMENT_TREE).apply {
                if (Build.VERSION.SDK_INT >= 26) {
                    putExtra(
                        ...
                        DocumentFileCompat.createDocumentUri(PRIMARY)
                    )
                }
            }
        ...
        fun hasStorageAccess(
            ...
            return (requiresWriteAccess && hasStoragePermission(context) || !requiresWriteAccess && hasStorageReadPermission(
                context
            ))
                    && DocumentFileCompat.getAccessibleRootDocumentFile(
                context,
                fullPath,
                requiresWriteAccess
            ) != null
